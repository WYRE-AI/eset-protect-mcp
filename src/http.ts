...
import type { EsetProtectRegion } from './types.js';
import { isValidRegion } from './client.js';
...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === '/health') {
      ...
      // mode, credentials arrive per-request via X-EsetProtect-* headers,
    ...
    const clientId = isGatewayMode ? (req.headers['x-esetprotect-client-id'] as string | undefined) : undefined;
    const clientSecret = isGatewayMode ? (req.headers['x-esetprotect-client-secret'] as string | undefined) : undefined;
    const rawRegion = isGatewayMode ? (req.headers['x-esetprotect-region'] as string | undefined) : undefined;
    const region: EsetProtectRegion | undefined = rawRegion && isValidRegion(rawRegion) ? rawRegion : undefined;
    ...
    if (clientId && clientSecret) {
      await runWithCredentials({ clientId, clientSecret, region }, handle);
    } else {
      await handle();
    }
  });
